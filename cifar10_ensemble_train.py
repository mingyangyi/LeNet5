...
MODEL_SAVE_PATH="D:/paper/Two new methods on manifold optimization/code/cifar10/tmp"
FILE_SAVE_PATH="D:/paper/Two new methods on manifold optimization/code/cifar10"
MODEL_NAME="cnn_for_cifar10"

BATCH_SIZE=128#批量大小
LEARNING_RATE_BASE=1.0#基础学习率，书中的基础学习率设置的太大了，高度非线性跳，导致梯度会很大，学习率一定要小。
LEARNING_RATE_GRASSMANN= 1.0
LEARNING_RATE_OBLIQUE= 1.0
LEARNING_RATE_DECAY=0.999#衰减学习率
REGULARIZATION_RATE=0.0001#正则项损失系数
TRAINING_STEPS=400#训练轮数
MOVING_AVERAGE_DECAY=0.99#滑动平均衰减率
GRAD_CLIP=1.0
DELTA=tf.convert_to_tensor(0.99,dtype=tf.float32)
EPOCH = 10

def train(LEARNING_RATE_BASE,MODEL_SAVE_PATH,FILE_SAVE_PATH):
    data, labels = reader.unpickle(reader.file)
    file_path_loss_g = os.path.join(FILE_SAVE_PATH, ('loss_g_' + str(LEARNING_RATE_GRASSMANN) + '.txt'))
    file_path_loss_o = os.path.join(FILE_SAVE_PATH, ('loss_o_' + str(LEARNING_RATE_OBLIQUE) + '.txt'))
    ...
    file1_path_g = os.path.join(FILE_SAVE_PATH, ('accuracy_g_' + str(LEARNING_RATE_GRASSMANN) + '.txt'))
    file1_path_o = os.path.join(FILE_SAVE_PATH, ('accuracy_o_' + str(LEARNING_RATE_OBLIQUE) + '.txt'))
    ...
    x=tf.placeholder(tf.float32,shape=[None,cifar10_ensemble.INPUT_NODE],name="x-input")
    y_=tf.placeholder(tf.float32,shape=[None,cifar10_ensemble.OUTPUT_NODE],name="y-output")
    x_reshaped=tf.reshape(x,[-1,cifar10_ensemble.IMAGE_SIZE,cifar10_ensemble.IMAGE_SIZE,cifar10_ensemble.NUM_CHANNELS])
    ...
    learning_rate_g=tf.train.exponential_decay(LEARNING_RATE_GRASSMANN,global_step,50000/BATCH_SIZE,LEARNING_RATE_DECAY)
    learning_rate_o = tf.train.exponential_decay(LEARNING_RATE_OBLIQUE, global_step, 50000 / BATCH_SIZE,
                                               LEARNING_RATE_DECAY)
...
#########################################################################################################3
    with tf.variable_scope('layer1-conv1', reuse=True):
        ...
        weights_biases_g_layer1 = tf.add(
            -1 * learning_rate_g * tf.convert_to_tensor(weights_grad_g_base_biases_layer1[0], tf.float32), conv1_biases_g)
        weights_biases_o_layer1 = tf.add(
            -1 * learning_rate_o * tf.convert_to_tensor(weights_grad_o_base_biases_layer1[0], tf.float32), conv1_biases_o)
    ...
    with tf.variable_scope('layer3-conv2', reuse=True):
        ...
        weights_biases_g_layer3 = tf.add(
            -1 * learning_rate_g * tf.convert_to_tensor(weights_grad_g_base_biases_layer3[0], tf.float32), conv2_biases_g)
        weights_biases_o_layer3 = tf.add(
            -1 * learning_rate_o * tf.convert_to_tensor(weights_grad_o_base_biases_layer3[0], tf.float32), conv2_biases_o)
    ...
    with tf.variable_scope('layer5-fc1', reuse=True):
        ...
        weights_biases_g_layer5 = tf.add(
            -1 * learning_rate_g * tf.convert_to_tensor(weights_grad_g_base_biases_layer5[0], tf.float32), fc1_biases_g)
        weights_biases_o_layer5 = tf.add(
            -1 * learning_rate_o * tf.convert_to_tensor(weights_grad_o_base_biases_layer5[0], tf.float32), fc1_biases_o)
    ...
    with tf.variable_scope('layer6-fc2', reuse=True):
        ...
        weights_biases_g_layer6 = tf.add(
            -1 * learning_rate_g * tf.convert_to_tensor(weights_grad_g_base_biases_layer6[0], tf.float32), fc2_biases_g)
        weights_biases_o_layer6 = tf.add(
            -1 * learning_rate_o * tf.convert_to_tensor(weights_grad_o_base_biases_layer6[0], tf.float32), fc2_biases_o)
    ...
    _1 = tf.assign(conv1_weights_g_tmp_layer1, gutils.unit(tf.reshape(weights_g_layer1, shape=dim_layer1)))
    _2 = tf.assign(conv1_weights_o_tmp_layer1, gutils.unit(tf.reshape(weights_o_layer1, shape=dim_layer1)))
    _3 = tf.assign(conv2_weights_g_tmp_layer3, gutils.unit(tf.reshape(weights_g_layer3, shape=dim_layer3)))
    _4 = tf.assign(conv2_weights_o_tmp_layer3, gutils.unit(tf.reshape(weights_o_layer3, shape=dim_layer3)))
    _5 = tf.assign(fc1_weights_g_tmp_layer5, gutils.unit(tf.reshape(weights_g_layer5, shape=dim_layer5)))
    _6 = tf.assign(fc1_weights_o_tmp_layer5, gutils.unit(tf.reshape(weights_o_layer5, shape=dim_layer5)))
    _7 = tf.assign(fc2_weights_g_tmp_layer6, gutils.unit(tf.reshape(weights_g_layer6, shape=dim_layer6)))
    _8 = tf.assign(fc2_weights_o_tmp_layer6, gutils.unit(tf.reshape(weights_o_layer6, shape=dim_layer6)))
...
######################################################################################################################
    ...
    #****************************************************************
                ...
                n_value = sess.run(n, feed_dict={x: xs, y_: ys, times: float(u)})
    ...
    ##########################################################################################################
        ...
        #xs ,ys= reader.test_unpickle(reader.file)
...
def main(argv=None):
    train(LEARNING_RATE_BASE,MODEL_SAVE_PATH,FILE_SAVE_PATH)
