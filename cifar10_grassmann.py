...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer1-conv1_grassmann'):
        ...
        conv1_biases_g = tf.get_variable("biases_g", shape=[CONV1_DEEP], initializer=tf.constant_initializer(1.0))
    ...
    with tf.name_scope('layer2-pool1_grassmann'):
        pool1_g=tf.nn.max_pool(relu1_g_grassmann,ksize=[1,3,3,1],strides=[1,2,2,1],padding='SAME')#第二个参数是步幅，第三个参数是步长
        pool1_batch_g_grassmann=batch_norm.batch_norm(pool1_g,scale=None)

    with tf.variable_scope('layer3-conv2_grassmann'):
        conv2_weights_g=tf.get_variable('weight_g',shape=[CONV2_SIZE,CONV2_SIZE,CONV1_DEEP,CONV2_DEEP],initializer=tf.truncated_normal_initializer(stddev=0.01))
    ...
    with tf.name_scope('layer4-pool2_grassmann'):
        pool2_g = tf.nn.max_pool(relu2_g_grassmann, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')  # 第二个参数是步幅，第三个参数是步长
    ...
    with tf.variable_scope('layer5-fc1_grassmann'):
        ...
        fc1_weights_g_tmp = tf.get_variable('weight_g_tmp',
                                        shape=[nodes, FC_SIZE], initializer=tf.truncated_normal_initializer(stddev=1.0))
    ...
    with tf.variable_scope('layer6-fc2_grassmann'):
        fc2_weights_g=tf.get_variable('weight_g',shape=[FC_SIZE,NUM_LABELS],initializer=tf.truncated_normal_initializer(stddev=0.01))
