...
import cifar10_grassmann
import cifar10_oblique
...
def train(LEARNING_RATE_BASE,MODEL_SAVE_PATH,FILE_SAVE_PATH):
    ...
    file_path_loss_oblique = os.path.join(FILE_SAVE_PATH, ('loss_oblique_' + str(LEARNING_RATE_BASE) + '.txt'))

    file1_path_grassmann = os.path.join(FILE_SAVE_PATH, ('accuracy_grassmann_' + str(LEARNING_RATE_BASE) + '.txt'))
    file1_path_oblique = os.path.join(FILE_SAVE_PATH, ('accuracy_oblique' + str(LEARNING_RATE_BASE) + '.txt'))
    ...
    y_g_grassmann = cifar10_grassmann.inference(x_reshaped,False,regularizer)
    y_o_oblique = cifar10_oblique.inference(x_reshaped,False,regularizer)
    ...
    learning_rate=tf.train.exponential_decay(LEARNING_RATE_BASE,global_step,50000.0/BATCH_SIZE,LEARNING_RATE_DECAY)
...
###########################################################################################################
    with tf.variable_scope('layer1-conv1_grassmann', reuse=True):
        ...
        weights_g_layer1 = optimize_function.apply_dense_on_grasssmann_g(GRAD_CLIP, grad_on_grassmann,
                                                                  weights_g, learning_rate, times, DELTA)
    ...
    with tf.variable_scope('layer3-conv2_grassmann', reuse=True):
        ...
        weights_g_layer3 = optimize_function.apply_dense_on_grasssmann_g(GRAD_CLIP, grad_on_grassmann
                                                                  , weights_g, learning_rate, times,
                                                                 DELTA)
    ...
    with tf.variable_scope('layer5-fc1_grassmann', reuse=True):
            ...
            weights_g_layer5 = optimize_function.apply_dense_on_grasssmann_g(GRAD_CLIP, grad_on_grassmann
                                                                      , weights_g, learning_rate, times, DELTA)
    ...
    with tf.variable_scope('layer6-fc2_grassmann', reuse=True):
        ...
        weights_g_layer6 = optimize_function.apply_dense_on_grasssmann_g(GRAD_CLIP, grad_on_grassmann,
                                                                  weights_g, learning_rate, times, DELTA)
...
############################################################################################################
    with tf.variable_scope('layer1-conv1_oblique', reuse=True):
        ...
        weights_o_layer1_o = optimize_function._apply_dense_on_oblique_o(GRAD_CLIP, grad_on_oblique
                                                                , weights_o, learning_rate, times,
                                                                DELTA)
    ...
    with tf.variable_scope('layer3-conv2_oblique', reuse=True):
        ...
        weights_o_layer3_o = optimize_function._apply_dense_on_oblique_o(GRAD_CLIP, grad_on_oblique,
                                                                weights_o, learning_rate, times,
                                                                DELTA)
    ...
    with tf.variable_scope('layer5-fc1_oblique', reuse=True):
        ...
        weights_o_layer5_o = optimize_function._apply_dense_on_oblique_o(GRAD_CLIP, grad_on_oblique
                                                                , weights_o,learning_rate, times, DELTA)
    ...
    with tf.variable_scope('layer6-fc2_oblique', reuse=True):
        ...
        weights_o_layer6_o = optimize_function._apply_dense_on_oblique_o(GRAD_CLIP, grad_on_oblique
                                                                , weights_o, learning_rate, times, DELTA)
