...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        ...
        conv1_weights_g=tf.get_variable("weight_g",shape=[CONV1_SIZE,CONV1_SIZE,NUM_CHANNELS,CONV1_DEEP],initializer=tf.truncated_normal_initializer(stddev=0.1))
        ...
        conv1_biases_g_tmp = tf.get_variable("biases_g_tmp", shape=[CONV1_DEEP], initializer=tf.constant_initializer(0.0))
        conv1_biases_o_tmp = tf.get_variable("biases_o_tmp", shape=[CONV1_DEEP], initializer=tf.constant_initializer(0.0))
    ...
    with tf.name_scope('layer2-pool1'):
        pool1_g=tf.nn.max_pool(relu1_g,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')#第二个参数是步幅，第三个参数是步长
        pool1_batch_g=batch_norm.batch_norm(pool1_g,scale=None)

        pool1_o = tf.nn.max_pool(relu1_o, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 第二个参数是步幅，第三个参数是步长
        pool1_batch_o = batch_norm.batch_norm(pool1_o, scale=None)
    #输出是14*14*32，池化不改变层数
    #第三层卷积层，步幅为5，步长为1，深度为64，全0填充，输出为14*14*64
    with tf.variable_scope('layer3-conv2'):
        conv2_weights_g=tf.get_variable('weight_g',shape=[CONV2_SIZE,CONV2_SIZE,CONV1_DEEP,CONV2_DEEP],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.name_scope('layer4-pool2'):
        pool2_g = tf.nn.max_pool(relu2_g, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 第二个参数是步幅，第三个参数是步长
        pool2_batch_g=batch_norm.batch_norm(pool2_g,scale=None)

        pool2_o = tf.nn.max_pool(relu2_o, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 第二个参数是步幅，第三个参数是步长
    ...
    with tf.variable_scope('layer6-fc2'):
        fc2_weights_g=tf.get_variable('weight_g',shape=[FC_SIZE,NUM_LABELS],initializer=tf.truncated_normal_initializer(stddev=0.1))
