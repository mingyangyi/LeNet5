...
def inference(input_tensor,train,regularizer):
    ...
    with tf.name_scope('layer2-pool1_oblique'):
        pool1_o = tf.nn.max_pool(relu1_o_oblique, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 第二个参数是步幅，第三个参数是步长
    ...
    with tf.name_scope('layer4-pool2_oblique'):
        pool2_o = tf.nn.max_pool(relu2_o_oblique, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 第二个参数是步幅，第三个参数是步长
    ...
    with tf.variable_scope('layer6-fc2_oblique'):
        fc2_weights_o = tf.get_variable('weight_o', shape=[FC_SIZE, NUM_LABELS],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1,seed=8))
